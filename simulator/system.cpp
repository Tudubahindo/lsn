...
void System ::write_configuration() {
    ofstream coutf;
    string file = "config.out.xyz";
    if (_restart == false && _sim_type == 0) {
        file = "config.in.xyz";
    }
    if (_sim_type < 2) {
        coutf.open(_filename + file);
        if (coutf.is_open()) {
            coutf << _npart << "\n";
            coutf << "#Comment!"
                  << "\n";
            ...
            cerr << "PROBLEM: Unable to open " << file
                 << "\n";
        coutf.close();
...
void System ::read_configuration() {
    ifstream cinf;
    string file = "config.in.xyz";
    if (_restart == false && _sim_type == 0) {
        file = "config.crystal.xyz";
    }
    cinf.open(_filename + file);
    if (cinf.is_open()) {
        string comment;
        string particle;
        double x, y, z;
